...
class StyleTransfer(object):
    def __init__(self, content_img, style_img, img_width, img_height):
        ...
        self.content_w = 1 #
        self.style_w = 1000 #changed from 100
    ...
    def _content_loss(self, P, F):
        ...
        ###############################
        ## TO DO
        squared_diff = tf.square(F-P)
        sum_squared_diff = tf.reduce_sum(squared_diff)
        s = P.size
        self.content_loss = sum_squared_diff/(4*s)
        ###############################
    ...
    def _gram_matrix(self, F, N, M):
        ...
        F_transposed = tf.transpose(F_reshaped)
        return tf.matmul(F_transposed, F_reshaped)
        ###############################
    ...
    def _single_style_loss(self, a, g):
        ...
        sum_squared_diff = tf.reduce_sum(tf.square(G-A))
        E_l = sum_squared_diff/(4*N**2*M**2)
    ...
    def _style_loss(self, A):
        ...
        #sess.run(self.input_img.assign(self.initial_img)) #should a different image be inside of assagn() here?
        #style_layers = sess.run([getattr(self.vgg, layer) for layer in self.style_layers])
        gen_img_style = [getattr(self.vgg, layer) for layer in self.style_layers]#sess.run([getattr(self.vgg, layer) for layer in self.style_layers]) #index into self.style_layers?
        num_layers = len(self.style_layers)
        E = [self._single_style_loss(A[i], gen_img_style[i]) for i in range(num_layers)]


        self.style_loss = sum([self.style_layer_w[i] * E[i] for i in range(num_layers)])
        ###############################
    ...
    def train(self, n_iters):
        skip_step = 1
        utils.safe_mkdir('checkpoints')  #I added this
        with tf.Session() as sess:
            
            ###############################
            ## TO DO: 
            ## 1. initialize your variables            Done
            ## 2. create writer to write your graph    Done
            ###############################
            ...
            ###############################
            ## TO DO: 
            ## 1. create a saver object                                 Done
            ## 2. check if a checkpoint exists, restore the variables   Done
            ##############################
            ...
            for index in range(initial_step, n_iters):
                ...
                if (index + 1) % skip_step == 0:
                    ...
                    gen_image, total_loss, summary = sess.run([self.input_img, self.total_loss, self.summary_op]) #where should it look for gen_image?
                    ###############################
...
if __name__ == '__main__':
    setup()
    machine = StyleTransfer('content/mouse_kidney_projection.jpg', 'styles/starry_night.jpg', 1487, 998) #Mona Lisa is 170x170
    machine.build()
    machine.train(200) 
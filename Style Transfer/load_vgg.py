...
class VGG(object):
    ...
    def conv2d_relu(self, prev_layer, layer_idx, layer_name):
        ...
        ###############################
        ## TO DO
        with tf.variable_scope(layer_name) as scope: #reuse=tf.AUTO_REUSE?
            W_numpy, b_numpy = self._weights(layer_idx, layer_name)
            kernel = tf.constant(W_numpy, name = 'kernel')
            biases = tf.constant(b_numpy, name = 'biases')
            conv = tf.nn.conv2d(prev_layer, filter = kernel, strides=[1, 1, 1, 1], padding='SAME')
    ...
    def avgpool(self, prev_layer, layer_name):
        ...
        ksize = 2
        stride = 2
        with tf.variable_scope(layer_name) as scope: #reuse?
            pool = tf.nn.avg_pool(prev_layer, 

%
%
%  LUMoS Spectral Unmixing Function for Imaris 
%
%  Tristan McRae 2018
...
%          <Item name="LUMoS Spectral Unmixing" icon="Matlab" tooltip="Split mixed fluorophores to each have their own channel">
...
%Set up progress bar
vProgressDisplay = waitbar(0,'LUMoS: Loading Data...');
...
function [] = spectral_unmixing_gui()%num_channels, num_timesteps, x_dim, y_dim, z_dim, mixed_image)
    ...
    h.f = figure('units','pixels','position',[700,500,250,100],...
                 'toolbar','none','menu','none', 'Name', 'LUMoS Task', 'NumberTitle', 'off');
...
function [] = k_means_gui()%k_means_gui(num_channels, num_timesteps, x_dim, y_dim, z_dim, mixed_image)
    ...
    h.f = figure('units','pixels','position',[700,500,width,height],...
                 'toolbar','none','menu','none', 'Name', 'LUMoS Settings', 'NumberTitle', 'off');
...
function [] = k_means_unmixing()%(num_channels, num_timesteps, x_dim, y_dim, z_dim, num_fluorophores_to_unmix, channels_to_unmix, mixed_image, replicates,...
    ...
    close all;    
    vProgressDisplay = waitbar(0.2,'LUMoS:Running K-Means...');
    %This really does take a long time for big datasets. Running on just
    %one z-slice could help that
    disp('Running LUMoS. The display may freeze but it is still working behind the scenes. This may take a few minutes for large datasets.')
    ...
    close all;
    vProgressDisplay = waitbar(0.5,'LUMoS:Finished Running K-Means...');
    pause(0.01) %let the display catch up

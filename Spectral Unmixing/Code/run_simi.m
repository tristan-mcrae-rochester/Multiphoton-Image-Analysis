...
if InSight
    channel_ranges = transpose([filter1])
    fluorophore_indices = [1]
    signatures = get_fluorophore_signature(fluorophores(fluorophore_indices), channel_ranges)
    channels_to_unmix = [1];
...
save_loc = 'D:\Projects\Local\Channel Unmixing\Results\simi_example.tif'; %strcat(fpath, fname);
